 ...
 * @param {number|string|null} targetRound - ???? ???? ???, ???? 'any'(???? ???? ???? ????)

 * @returns {number} ???????????????? ??(A~E 5???? = 5)
...
export function getUserConfirmedGameCountForRound(userId = null, targetRound = null) {
    ...
    if (!cleanAuthId || cleanAuthId === 'guest' || cleanAuthId === '??α???' || cleanAuthId === 'anonymous') {
    ...
    const countGames = (receipts) => {
        ...
        receipts.forEach(rc => {
            ...
            // master ?? ??o???(1235~1240) ??mock/placeholder ????
...
export function deduplicateReceipts(receiptList) {
    ...
    receiptList.forEach(item => {
        ...
        // Detect generic/placeholder serials (e.g. 'TR-??????', 'TR-????', 'TR-?????? ??????)

        const isGenericSerial = !serial || serial === 'TR-미발급' || serial === 'TR-확인불가' || serial === 'TR-스마트폰 자동생성' || serial.startsWith('TR-임시');
 ...
 * Receipt #1: 107114057514142041 (???? 5 #1, ??÷ 0??

 * Receipt #2: 106292723514142041 (V3.0 #2, ??÷ 0??

 * Receipt #3: 106292762114142041 (V4.0, ??÷ 0??

 * Receipt #4: 106292663514142041 (V3.0 #1, 10,000 KRW winner: Game B 5th + Game E 5th)

 * Receipt #5: 107114111414142041 (???? 5 #2, ??÷ 0??
...
export function buildDonghangLotteryQrUrl(round, combos = [], serial = '', existingRawUrl = '') {
    ...
    if (!serialStr || !/^\d{10,24}$/.test(serialStr) || serialStr.startsWith('TR-') || serialStr === 'TR-스마트폰 자동생성') {
...
export async function saveToLedger(round, combos, versionStr, user = null, qrMeta = null, isAsyncServerSync = true) {
    ...
    // ??? [???? 1] ???? ???? ??????1??1???? ???? ??(Cross-User Receipt Duplicate Detection)
        ...
        for (const otherUserId in state.allUsersPurchasesMap) {
            ...
            for (const receipt of otherRoundReceipts) {
                ...
                if (matchCount >= 3) {
                    ...
                    if (typeof showToast === 'function') showToast('데이터 저장 중 오류가 발생했습니다.', 4000);
    ...
    for (let i = 0; i < cleanCombos.length; i += chunkSize) {
        ...
        const chunkCombos = chunk.map((c, idx) => ({
            ...
            meta: { ...c.meta, name: labels[idx] || `${idx + 1} ????` },
        ...
        } else if (!vStr.includes('5????') && chunkCombos.length === 5) {

            vStr = `${vStr} (5????)`;

        } else if (!vStr.includes('????')) {

            vStr = `${vStr} (${chunkCombos.length}????)`;
        ...
        let algoName = 'V4.0 AI 예측 조합';

        if (vStr.includes('V3') || vStr.includes('AI')) {

            algoName = 'V3.0 ????긮?????????';

        } else if (vStr.includes('수동')) {

            algoName = '수동 선택 / 랜덤';

        } else if (vStr.includes('QR') || vStr.includes('????')) {
        ...
        return [

            {
                ...
                combos: [
                    ...
                    { numbers: [1, 5, 12, 19, 26, 34], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [2, 7, 15, 23, 30, 37], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [3, 8, 16, 24, 31, 38], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [4, 9, 18, 25, 32, 39], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [10, 13, 20, 27, 35, 40], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [11, 15, 21, 28, 33, 43], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [1, 8, 17, 24, 32, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [2, 12, 19, 26, 35, 45], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [3, 7, 16, 25, 33, 40], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 10, 18, 27, 34, 43], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [5, 11, 17, 23, 31, 42], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [3, 12, 19, 25, 35, 43], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [7, 13, 20, 27, 34, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [1, 10, 18, 26, 33, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 15, 21, 28, 37, 40], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [8, 16, 24, 30, 38, 43], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [2, 11, 19, 26, 32, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [5, 13, 20, 27, 35, 42], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [3, 15, 23, 31, 39, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [1, 12, 18, 25, 34, 40], meta: { name: 'E ????' }, stats: {} }
        ...
        return [

            {
                ...
                combos: [
                    ...
                    { numbers: [2, 12, 19, 26, 34, 43], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [5, 14, 21, 28, 36, 45], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [7, 15, 22, 29, 37, 41], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [4, 13, 20, 27, 35, 40], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [8, 16, 23, 31, 38, 43], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [1, 10, 17, 24, 32, 39], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [6, 17, 24, 32, 40, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [9, 16, 24, 32, 38, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [2, 12, 20, 29, 37, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 14, 22, 30, 39, 45], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [5, 12, 19, 26, 34, 41], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [3, 14, 21, 28, 36, 43], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [7, 13, 20, 27, 35, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [1, 15, 22, 29, 37, 43], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 16, 23, 30, 38, 40], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [6, 17, 24, 31, 39, 45], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [8, 17, 26, 32, 36, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [2, 10, 17, 24, 34, 41], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [5, 13, 20, 28, 35, 43], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [3, 16, 23, 30, 37, 40], meta: { name: 'E ????' }, stats: {} }
        ...
        return [

            {
                ...
                combos: [
                    ...
                    { numbers: [2, 9, 16, 20, 31, 40], meta: { name: 'A ????' }, stats: {} }, // 5?????? (2, 9, 16)

                    { numbers: [5, 12, 18, 26, 33, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [7, 14, 22, 29, 38, 41], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [3, 10, 17, 25, 36, 42], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [8, 15, 24, 30, 37, 43], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [2, 27, 34, 11, 23, 39], meta: { name: 'A ????' }, stats: {} }, // 5?????? (2, 27, 34)

                    { numbers: [1, 13, 19, 28, 35, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [4, 11, 22, 32, 39, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [6, 17, 23, 31, 38, 40], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [8, 15, 28, 33, 39, 42], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [5, 11, 17, 23, 31, 41], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [3, 12, 19, 25, 36, 43], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [7, 14, 20, 28, 37, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [1, 10, 18, 26, 35, 42], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 13, 22, 29, 38, 40], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [6, 15, 24, 30, 39, 43], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [8, 17, 23, 32, 36, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [1, 11, 20, 28, 33, 41], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [5, 14, 22, 31, 37, 42], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [3, 13, 19, 26, 35, 40], meta: { name: 'E ????' }, stats: {} }
        ...
        return [

            {
                ...
                combos: [
                    ...
                    { numbers: [2, 13, 18, 24, 30, 44], meta: { name: 'A ????' }, stats: {} }, // 5?????? (2, 13, 18)

                    { numbers: [2, 32, 38, 11, 25, 41], meta: { name: 'B ????' }, stats: {} }, // 5?????? (2, 32, 38)

                    { numbers: [6, 15, 21, 35, 39, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [5, 14, 23, 29, 36, 40], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [8, 16, 27, 31, 37, 43], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [13, 18, 42, 7, 26, 35], meta: { name: 'A ????' }, stats: {} }, // 5?????? (13, 18, 42)

                    { numbers: [3, 12, 20, 28, 34, 45], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [1, 10, 19, 27, 33, 41], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [4, 15, 23, 30, 39, 44], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [9, 17, 25, 33, 37, 45], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [1, 10, 24, 26, 35, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 15, 29, 30, 39, 40], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [6, 16, 27, 33, 34, 43], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [8, 17, 23, 31, 36, 45], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [1, 11, 20, 28, 33, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [5, 14, 25, 29, 37, 40], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [3, 15, 19, 26, 35, 41], meta: { name: 'E ????' }, stats: {} }
        ...
        return [

            {
                ...
                combos: [
                    ...
                    { numbers: [1, 11, 13, 25, 36, 38], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [6, 11, 23, 29, 33, 36], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [3, 4, 17, 20, 24, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [7, 8, 16, 30, 39, 44], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 10, 18, 23, 37, 38], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [7, 8, 24, 34, 36, 41], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [8, 9, 12, 35, 40, 45], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [2, 11, 21, 33, 34, 44], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [2, 12, 15, 36, 39, 41], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [3, 9, 16, 36, 41, 43], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [5, 9, 11, 12, 31, 32], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [2, 3, 14, 22, 38, 39], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [1, 13, 14, 19, 31, 38], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [4, 10, 15, 23, 24, 43], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [13, 15, 20, 27, 31, 35], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [3, 11, 15, 36, 40, 44], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [1, 3, 26, 32, 41, 44], meta: { name: 'B ????' }, stats: {} }, // 5?????? (1, 3, 26) -> 5,000??
                    { numbers: [2, 4, 16, 33, 38, 45], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [7, 20, 26, 35, 39, 40], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [1, 23, 33, 41, 42, 44], meta: { name: 'E ????' }, stats: {} }  // 5?????? (1, 33, 42) -> 5,000??(??????4????10,000??
...

            {
                ...
                combos: [
                    ...
                    { numbers: [2, 11, 18, 34, 39, 42], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [5, 11, 14, 24, 31, 32], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [8, 19, 20, 35, 39, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [3, 19, 22, 35, 44, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [12, 14, 26, 34, 37, 45], meta: { name: 'E ????' }, stats: {} }
        ...
        return [

            {
                ...
                combos: [
                    ...
                    { numbers: [3, 11, 17, 24, 33, 42], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [5, 15, 22, 29, 36, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [7, 13, 21, 28, 35, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [2, 10, 19, 27, 34, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [4, 16, 23, 30, 37, 41], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [8, 14, 25, 31, 38, 40], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [6, 17, 24, 32, 39, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [9, 15, 22, 30, 37, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [3, 12, 21, 29, 36, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [5, 13, 20, 28, 35, 42], meta: { name: 'E ????' }, stats: {} }
...

            {
                ...
                combos: [
                    ...
                    { numbers: [2, 11, 19, 26, 33, 41], meta: { name: 'A ????' }, stats: {} },

                    { numbers: [7, 14, 23, 30, 38, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [4, 10, 18, 25, 34, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [1, 15, 22, 29, 37, 45], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [11, 19, 20, 27, 35, 40], meta: { name: 'E ????' }, stats: {} } // 5?????? (11, 19, 20) -> 5,000??
                ]
...

            {
                ...
                combos: [
                    ...
                    { numbers: [11, 13, 18, 19, 25, 38], meta: { name: 'A ????' }, stats: {} }, // 5?????? (11, 13, 19) -> 5,000??
                    { numbers: [4, 15, 23, 31, 39, 44], meta: { name: 'B ????' }, stats: {} },

                    { numbers: [8, 16, 24, 32, 37, 43], meta: { name: 'C ????' }, stats: {} },

                    { numbers: [2, 11, 19, 28, 36, 42], meta: { name: 'D ????' }, stats: {} },

                    { numbers: [5, 14, 22, 30, 35, 41], meta: { name: 'E ????' }, stats: {} }
...
export function calculateLedgerFinancials(forceRefresh = false, explicitTarget = null) {
    ...
    const cacheKey = `${authId}_ALL_reg_${explicitTarget || 'none'}`;
        ...
        if (actualDraw && actualDraw.numbers) {
            ...
            flatCombos.forEach((combo, cIdx) => {
                ...
                if (rank > 0) {
                    roundPrize += prize;
                    roundHits[rank]++;
                    winningCombos.push({ nums, rank, prize, cIdx });
                }
            });
        }
...
export async function importImmutableUnifiedArchive(file) {
    ...
    try {
        ...
        for (const r in purchasesToRestore) {
            ...
            if (!isNaN(roundNum) && roundNum > 0 && Array.isArray(purchasesToRestore[r])) {
                ...
                cleanLedger[roundNum] = purchasesToRestore[r].map(p => ({
                    ...
                    version: p.version || 'QR ????????????(5????)',

                    algoName: p.algoName || 'V4.0 AI 예측 조합',
        ...
        await saveLedgerDirectly(mergedLedger, null, '????o??÷??? ???????????????? ?????? ??????100% ?????????????? ??????????????');
...
export async function clearEntireLedger() {
    ...
    await saveLedgerDirectly({}, null, '??? ???? ??? ???????? ???????? ???????????????????? (1239??????????????????)');
...
export async function restoreFromReceiptTrash(trashId, currentAuthId) {
    ...
    const restoredPurchase = {
        ...
        version: item.version || 'QR ????????????(5????)',

        algoName: item.algoName || '???? QR ??????/ ????',
    ...
    await saveLedgerDirectly(uLedger, targetUser, `??? ??${round}???? ???? ????????????????????????????????????????`);
...
export function getUserConfirmedWinningsAuditTrail(userId) {
    ...
    try {
        ...
        rounds.forEach(roundNum => {
            ...
            const drawDate = actualDraw?.date || actualDraw?.drwNoDate || `??${roundNum}????`;
            ...
            receipts.forEach((rcpt, rcptIdx) => {
                ...
                const algoVersion = rcpt.version || rcpt.algoName || 'AI ???? ??????o';
                    ...
                    combos.forEach((combo, comboIdx) => {
                        ...
                        if (rank > 0) {
                            ...
                            winningItems.push({
                                ...
                                qrRaw: qrRaw ? (qrRaw.length > 24 ? qrRaw.slice(0, 24) + '...' : qrRaw) : '?????????????? ????',
...
export async function toggleRoundLock(round, forceState = null, user = null) {
    ...
    if (!isAdmin) {
        ...
        if (typeof showToast === 'function') showToast('??? ???? ??u ?????? ?????? ???? ?????????');
...
export async function calculateAllUsersTotalFinancials() {
    return calculateLedgerFinancials(false, 'all');
}
